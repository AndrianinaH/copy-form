...
class FormCopyPopup {
  ...
  updateStatus(type, text, icon = null) {
    ...
    if (icon) {
      ...
      const icons = {
        success:
          '<circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/><path d="m9 12 2 2 4-4" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
        loading:
      ...
      this.statusIcon.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">${
  ...
  async loadRecentForms() {
    try {
      ...
      console.error(
        'Erreur lors du chargement des formulaires récents:',
        error
      );
    }
  }
  ...
  renderRecentForms(forms) {
    ...
    this.recentList.innerHTML = forms
      .map(
        (form) => `
            <div class="recent-item" data-form-id="${form.id}">
                <div class="recent-item-content">
                    <div class="recent-item-title">${this.truncateText(
                      form.title,
                      40
                    )}</div>
                    <div class="recent-item-meta">
                        ${form.fieldCount} champs • ${this.formatRelativeTime(
          form.timestamp
        )}
                    </div>
                </div>
                <button class="delete-btn" data-form-id="${
                  form.id
    ...
    this.recentList.querySelectorAll('.recent-item').forEach((item) => {
      ...
      content.addEventListener('click', () =>
        this.loadStoredForm(item.dataset.formId)
      );
  ...
  async deleteStoredForm(formId) {
    try {
      ...
      const itemToRemove = this.recentList.querySelector(
        `[data-form-id="${formId}"]`
      );
      if (itemToRemove) {
        itemToRemove.remove();
      ...
      if (currentResult.currentFormData) {
        const currentFormResult = await chrome.storage.local.get([
          `form_${formId}`,
        ]);
      ...
      this.showToast(
        'success',
        'Formulaire supprimé',
        "Le formulaire a été retiré de l'historique"
      );
    } catch (error) {
      this.showToast(
        'error',
        'Erreur',
        'Impossible de supprimer ce formulaire'
      );
    }
  }
  ...
  async exportCurrentFormData() {
    try {
      ...
      if (result.currentFormData) {
        ...
        console.log(
          '💡 Sélectionnez le JSON ci-dessus et copiez-le avec Ctrl+C'
        );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log(
    '   chrome.storage.local.get(["currentFormData"], (r) => console.log(JSON.stringify(r.currentFormData, null, 2)))'
  );
  console.log('');
});
