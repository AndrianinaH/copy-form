    ...
    if (message.action === 'fillForm') {
        try {
            ...
            if (success) {
                ...
                sendResponse({ success: false, error: 'Échec du remplissage du formulaire' });
            }
        } catch (error) {
            ...
            if (element && (element.type === 'select-one' || element.multiple === false)) {
                ...
                for (let i = 0; i < element.options.length; i++) {
                    if (element.options[i].value === fieldValue.value) {
                        ...
                        console.log(`✅ Sélectionné select[${fieldName}] = "${fieldValue.selectedText}"`);
                        break;
                    }
                }
            }
        ...
        if (fieldValue && typeof fieldValue === 'object' && fieldValue.value !== undefined) {
            ...
            if (element) {
                ...
                if (selectizeInstance) {
                    ...
                    console.log(`✅ Sélectionné Selectize jQuery[${fieldName}] = "${fieldValue.selectedText}"`);
                }
            }
        }
    });
